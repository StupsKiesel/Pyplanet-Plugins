...
from pyplanet.apps.custom.chatsymbols.views.toolbar import ToolbarView, ToolbarToggle
...
class ToolbarPlayer:
	def __init__(self, app):
		...
		:type app: pyplanet.apps.custom.chatsymbols.app.Player
		"""
		self.app = app
		...
		self.setting_enable_toolbar = Setting(
			'enable_chatbar', 'Enable the Player chatbar', Setting.CAT_DESIGN, type=bool,
			description='Enable and show the chatbar for Players.',
			default=True
		)
	...
	async def on_start(self):
		...
		await self.instance.command_manager.register(
			Command(command='chatbar', target=self.toggle_toolbar, admin=False, description='Toggle the Player chatbar')
		)
	...
	async def toggle_toolbar(self, player, *args, **kwargs):
		if self.player_dict[player.login] == False:
			...
			message = '$s$o$fffchatsymbols $z$fffcreated by StupsKiesel & MosKi'
